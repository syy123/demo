...
def signal_equation__channel_process_complex(n,delay,power,flag): #,moduflag
    ...
    ImpulseObject = ImpulseForming(20, 80, 100, -4, 4, 0.5, 5);
    ...
    data_modu = np.cos(2*np.pi*10*t)
    pilot_add_data_1 = data*data_modu
    if moduflag == 0:
       pilot_data = pilot_add_data
    else:
       pilot_data = pilot_add_data_1 
    '''
    #cs = ChannelSource()
    ...
    h = np.zeros(500, np.float64) # 自适应滤波器长度
    ...
    u, h = nlms_numpy.nlms(pilot_out, d, h, 0.1)
