...
class ChannelSource(wx.Panel):
    ...
    def ImpulseChoiceBox(self):
        self.impulseChoiceBox = wx.ComboBox(self, -1,  choices = GetImpulseChoiceList())
    ...
    def ChoiceBoxSwitchFunc(self, event):
        ...
        if self.impulseChoiceBox.GetSelection() == 0:
        ...
        #if self.impulseChoiceBox.GetLabelText() == PartialBoxName():  
        elif self.impulseChoiceBox.GetSelection() == 1:
        ...
        #if self.impulseChoiceBox.GetLabelText() == GaussBoxName():
        elif self.impulseChoiceBox.GetSelection() == 2:
            ...
            self.left_vsizer.Hide(self.rectRatioBox)
            self.left_vsizer.Layout()
        #elif self.impulseChoiceBox.GetLabelText() == Rectangle():
        #if self.impulseChoiceBox.GetLabelText() == Rectangle():
        elif self.impulseChoiceBox.GetSelection() == 3:
    ...
    def OnStart(self, event):  
        if self.impulseChoiceBox.GetCurrentSelection() == -1:
            ...
            self.channel_in = []  
            for i in range(len(self.modulateReal)):
                self.channel_in.append(self.modulateReal[i])
                self.channel_in.append(self.modulateImage[i])
