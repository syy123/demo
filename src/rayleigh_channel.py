...
def RAYLEIGH(B_number,Fs,n,delay,power,fdts,s):
    ...
    for i in range(0,len(channel_out),2):
        channel_out_real.append(channel_out[i])
        channel_out_imag.append(channel_out[i+1])
        channel_out_complex.append(complex(channel_out_real[-1], channel_out_imag[-1]))
    ...
    demo_data_out = demodulate(channel_out_real,channel_out_imag,Fs)   #星座图
    ...
    return channel_out_complex,spec[0],demo_data_out[0],sf,r,angle,t1,spec[1],demo_data_out[1],rt # 信道输出
#频谱
...
#解调，星座图
def demodulate(real_data,imag_data,Fs):
    data_real_out = []
    data_imag_out = []

    for i in range(0, len(imag_data), Fs):
